...
(* I think that if (O, l1, l2) is not a persp_data, then you want
perspectivity (O, l1, l2) not to be undefined, but to be the map \<lambda> P . undefined;
that should make proving projectivity_not_nice a lot easier *)
(*Nick: DONE*)
...
lemma perspectivity_nice2: 
  fixes d
  assumes "perspectivity d \<noteq> (\<lambda> P . undefined)"
...
(* I'd insist that is_proj_data [stuff] = true for each of these cases; I'd also rule out "projectivity []" -- I'd make it undefined.
  and for   *)
(* Nick: DONE*)
...
(* I'd call this "is_proj_data_non_empty" or even "proj_data_is_nonempty" *)
...
(* and this would be "proj_data_starts_with_persp_data" *)
...
next
  ...
  proof -
    ...
    proof -
      ...
      have "persp_range a = proj_domain qs" using ah h1 proj_domain_cons 
          is_proj_data.simps(2) proj_domain.simps(1) projectivity.cases by metis
      then have "((perspectivity a P) \<in> Points) \<and> ((perspectivity a P) \<lhd> proj_domain qs)" 
        using p3 by metis
      then have "(projectivity qs (perspectivity a P) \<in> Points \<and> projectivity qs (perspectivity a P) \<lhd> proj_range qs)"
        using h0 by (metis ah Cons.IH h1 is_proj_data.simps(2) list.exhaust)
      then show ?thesis using p3a by auto
      qed
    qed
    show ?case by (metis Cons.prems(1) list.exhaust p1 p2 proj_range.simps(2) projectivity.simps(1))
  qed
qed
...
(* in all these cases, I'd make the third case not be "undefined" (i.e., SOME function from p to p, 
but we don't know which one), but instead be (\<lambda> P . undefined) *)
...
(* I really feel as if this ought to be easier, like ...
just CONSTRUCT the inverse projectivity by (1) inverting every perspecitivity data (so that (O, k, m)
becomes (O,m,k), and (2) reversing the resulting list of perspectivities. Then you have to show
that this new thing satisfies "is_proj_data", and then that it's actually the inverse of f. 

One way to do this is to say that if A B C D are perspectivities, and A', B', C', D' are the "inverted" 
perspectivities of those, then projectivity ([A,B,C,D][D',E, F]) == projectivity([A,B,C,E F]) (and any generalization of that). 
The one tricky case is that projectivity [A A'] CANNOT be simplified to projectivity[] (because that's undefined). But you can 
show that projectivity[A A'] is exactly equal to the identity (when restricted to l1). 
*)
...
(* Nice! I like the explicit construction. *)
...
(* I know this is a proof, and you're only going to do it once, but it's just crying
out for a few lemmas *)
