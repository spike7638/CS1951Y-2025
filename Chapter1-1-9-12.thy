...
theorem contains_implies_liesOn:
  ...
  shows "P liesOn m"
proof -
  show ?thesis using assms contains_def by auto
qed
...
proof -
  ...
  next
    case lm
    have 2: "\<not> (\<exists> P. P \<in> Points \<and> P \<lhd>  m \<and> P \<lhd>  k)" using 1 assms lm by auto
    have 3: "\<not> (\<exists> P. P \<in> Points \<and> P \<lhd>  l \<and> P \<lhd>  k)" using 2 lm by auto
    have 4: "l \<in> Lines" using assms parallel_def by auto
    have 5: "k \<in> Lines" using assms parallel_def by auto
    then show ?thesis unfolding parallel_def using 3 4 5 assms by auto
  next
    case mk
    have 6: "\<not> (\<exists> P. P \<in> Points \<and> P \<lhd>  m \<and> P \<lhd>  l)" using 0 assms mk by auto
    have 7: "\<not> (\<exists> P. P \<in> Points \<and> P \<lhd>  k \<and> P \<lhd>  l)" using 6 mk by auto
    have 8: "k \<in> Lines" using assms parallel_def  by auto
    have 9: "l \<in> Lines" using assms parallel_def  by auto
    then show ?thesis unfolding parallel_def using 7 8 9 assms by auto
  next
    case neither
    then show ?thesis using assms 0 1 parallel_def a2d by metis
  qed
qed
...
proof -
  show ?thesis unfolding reflp_on_def parallel_def by auto 
qed

lemma parallel_sym:
  shows "symp parallel" 
proof -
  show ?thesis unfolding symp_def parallel_def by auto
qed

lemma parallel_trans:
  shows "transp_on Lines parallel" 
proof -
  show ?thesis unfolding transp_on_def using parallel_transitive by blast
qed
