...
proof -
  ...
  show "harmonic_quadruple A' B' D' C'"
    ...
    by (smt (verit) A'_meet B'_meet \<open>(B \<bar> C' \<sqdot> Or \<bar> A) \<bar> B = B \<bar> C'\<close> cquadrangle_OrXBD
    distinct4_def harmonic_quadruple_def is_persp_data.simps join_properties1
    persp_data)
qed
